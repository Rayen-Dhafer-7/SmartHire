    ...
    <div class="card-body">
      <div class="d-flex justify-content-between align-items-start mb-2">
        <div class="d-flex align-items-center gap-3">
          ...
          <div v-if="post.logoUrl" class="company-logo d-flex align-items-center justify-content-center rounded border overflow-hidden">
      ...
      <p class="text-muted small mb-3">
        <span class="me-3">
          <i class="bi bi-clock"></i> Applied: {{ post.appliedDate }}
        </span>
        <span v-if="post.deadline_message" :class="getDeadlineClass(post.deadline_message)" class="fw-bold">
          <i class="bi bi-calendar-check"></i> {{ post.deadline_message }}
        </span>
      </p>
      ...
      <div v-if="post.myScore >= 0" class="mt-4 pt-3 border-top">
        <div class="d-flex align-items-center justify-content-between">
            <div class="d-flex align-items-center gap-3">
              <div class="display-6 fw-bold" :class="getScoreTextColor(post.myScore)">
                {{ post.myScore }}%
              </div>
              <div>
                <div class="fw-bold text-dark">Your Score</div>
                <div class="text-muted small">
                  {{ post.test?.total_correct || 0 }}/{{ post.test?.total_questions || 12 }} correct
                </div>
              </div>
            </div>
            
            <div class="progress flex-grow-1 mx-4" style="height: 10px; max-width: 200px;">
              <div class="progress-bar" 
                  role="progressbar" 
                  :class="getScoreColor(post.myScore)"
                  :style="{ width: post.myScore + '%' }" 
                  :aria-valuenow="post.myScore" 
                  aria-valuemin="0" 
                  aria-valuemax="100">
              </div>
            </div>
...
const getDeadlineClass = (message) => {
  if (!message) return 'text-secondary';
  ...
  // Ending Soon - Orange/Warning
  if (message.includes('Ending')) {
    return 'text-warning';
  }
  
  // Expired or Terminated - Red
  if (message.includes('Expired') || message === 'Terminated') {
    return 'text-danger';
  }
...
.bg-warning { background-color: #b45309 !important; }
